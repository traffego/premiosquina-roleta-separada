...
class Main extends DBConnection
{
    ...
    public function place_order()
    {
        ...
        if (flock($lock, LOCK_EX)) {
            ...
            $query =
                'SELECT discount_qty, enable_discount, discount_amount, enable_cumulative_discount, enable_sale, sale_qty, sale_price, status, qty_numbers, pending_numbers, paid_numbers, date_of_draw, first_purchase_enabled, first_purchase_free_qty, price, roleta AS prod_roleta, box AS prod_box, roleta_cotas_por_giro FROM product_list WHERE id = \'' .
                $product_id .
                '\'';
            ...
            if ($result && 0 < $result->num_rows) {
                ...
                $prod_roleta = (int)$row["prod_roleta"];
                $prod_box    = (int)$row["prod_box"];
                $roleta_cotas_por_giro = (int)$row["roleta_cotas_por_giro"];
            }
            ...
            // Calcular quantos giros de roleta/caixa o pedido gera
            $roleta_giros = 0;
            if (!empty($prod_roleta) || !empty($prod_box)) {
                if ($roleta_cotas_por_giro > 0) {
                    $roleta_giros = (int)floor($quantity / $roleta_cotas_por_giro);
                } else {
                    $roleta_giros = 1; // comportamento legado: 1 giro fixo
                }
            }
